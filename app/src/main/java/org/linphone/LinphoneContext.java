...
import static ribo.phone.Info.initMC74environ;
import static ribo.phone.Info.localSSM;
import static ribo.phone.Info.phSHC;
import static ribo.phone.Info.audSvc;
import static ribo.phone.Info.setAudioOutput;
...
import com.meraki.androidapi.media.ForceUseModes;
...
import org.linphone.core.tools.*;
...
public class LinphoneContext {
    ...
    private final LoggingServiceListener mJavaLoggingService = new LoggingServiceListener() {
        @Override
    ...
    public LinphoneContext(Context context) {
        ...
        System.out.println(
          "LinphoneContext constrtructor: forced debug on in LinphoneContext, was "
            + isDebugEnabled);
        isDebugEnabled = true; // WG
    ...
    } // end of LinphoneContext constructor
    ...
    public void start(boolean isPush) {
        ...
        // Get the SSM object MC74.mainFooter, populate 'footer' LinearLayout
        prt(SSMutil.getTimestamp()+" LinphoneContext.start: minimalSSMinit, SSMcmd.cmdHndlr setup");
        ...
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                // ctxTr = svrCmdReply(localSSM, "GETOBJ MC74");
                if (ctxTr == null) {
                  prt("LinphoneContext.start: call initMC74environ");
                  initMC74environ(getApplicationContext());
                }
                Tree tr = svrCmdReply(localSSM, "WRT MC74 POLLEVENTS /dev/input/event0");
                if (tr == null) {
                    prt("!!LinphoneContext.start: svrCmdReply to SSM server failed.");
                } else {
                    // Start capturing Hookswitch onhook, offhook events
                    prt("LinphoneContext.start.events0: " + tr.list());
                    tr.release();

                    // Start capturing Button events, VolUp, VolDown, Mute buttons
                    tr = svrCmdReply(localSSM, "WRT MC74 POLLEVENTS /dev/input/event1");
                    prt("LinphoneContext.start.events1: " + tr.list());
                    tr.release();

                    // Start capturing touchscreen touch events
                    tr = svrCmdReply(localSSM, "WRT MC74 POLLEVENTS /dev/input/event2");
                    prt("LinphoneContext.start.events2: " + tr.list());
                    tr.release();
                }

                // Convenience for debug add ctx to vars.  However this causes lots of
                // stuff to happen like <clinit> of ribo.ssm.Phone and EvalExp, so
                // don't do it at the moment.
                //EvalExp.varTable.put("ctx", Info.class);  // Make 'ctx' var available to the EvalExp debug tool
            } // end of run
        });

        // Initialize audio system to use speakerphone speaker
        // Devices are: FORCE_NONE, FORCE_SPEAKER, FORCE_HANDSET, FORCE_???
        audSvc = (AudioManager) getApplicationContext().getSystemService(Context.AUDIO_SERVICE);

        ForceUseModes.ForceUse dev = ForceUseModes.ForceUse.FORCE_SPEAKER;
        setAudioOutput(ForceUseModes.MediaStrategy.FOR_COMMUNICATION, dev);
        setAudioOutput(ForceUseModes.MediaStrategy.FOR_MEDIA, dev);
        setAudioOutput(ForceUseModes.MediaStrategy.FOR_SYSTEM, dev);
