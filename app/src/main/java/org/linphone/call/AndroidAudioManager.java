...
public class AndroidAudioManager {
        ...
        mListener =
                new CoreListenerStub() {
                    ...
                    public void onCallStateChanged(
                        ...
                        prt("Lp.AAudManger.onCallStateChanged: "+state.toString()
                          +" "+message);
                        // Notify SSM server of call state changes
                        Tree tr = new NB();
                        tr.changeValue("WRT MC74 CALLSTATE " + state + " " + message);
                        tr.add("FROM", call.getRemoteAddressAsString());
                        tr.add("TO", call.getToAddress().asString());
                        tr.add("DIR", call.getDir().toString());
                        tr.add("REMOTEUSERAGENT", call.getRemoteUserAgent());
                        tr.add("UNIQUEID", call.getRemoteAddressAsString());  // Sort of unique id
                        Tree respTr = ssmCmdReply(null, tr);
                        tr.release();
                        ...
                        if (state == Call.State.Connected) {
                            if (false && core.getCallsNb()==1) {  //--Audio modes set by SSM  OffHook()
                            ...
                            // WL Hide MC74 app, the call ended, resume whatever android app was  running
                            prt("AndroidAudioManager.onCallStateChanged: END, UID hide MC74, show prev app");
                        }
    ...
    private synchronized void startRinging(Address remoteAddress) {
        ...
        try {
            boolean vibrate = mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_VIBRATE
            ...
            if (tr.select("CONFIG")) {
                ringtone = tr.value("RINGTONE");
                if (tr.select("RINGVIBRATE")) {
                    vibrate = true;  // Presence of 'ringVibrate' means vibate
                }
            }
            tr.release();

            if (vibrate && mVibrator != null) {
            ...
            if (mRingerPlayer == null) {
                ...
                if (ringtone == null) {
                    LinphonePreferences.instance().getRingtone(
                ...
                try {
                    if (ringtone.startsWith("content://")) {
                        ...
                        if (ringtone.indexOf('/') < 0) {
                            ringtone = "/sdcard/ssm/audio/"+ringtone;
                        }
    ...
    private void routeAudioToSpeakerHelper(boolean speakerOn) {
        Log.w("!!use ssm [AndroidAudioManager] Routing audio to " + (speakerOn ? "speaker" : "earpiece"));
