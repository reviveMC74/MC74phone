...
class AsyncContactsLoader extends AsyncTask<Void, Void, AsyncContactsLoader.AsyncContactsData> {
    ...
    protected AsyncContactsData doInBackground(Void... params) {
        ...
        if (ContactsManager.getInstance().hasReadContactsAccess()) {
            ...
            // RAF Allowing selection to be set below, causes:
            // android.database.sqlite.SQLiteException: no such column: in_default_directory (code
            // 1): , while compiling: SELECT DISTINCT contact_id, display_name, mimetype, starred,
            // data1, data2, data3, data4 FROM view_data data WHERE (1) AND ((in_default_directory
            // == 1))
