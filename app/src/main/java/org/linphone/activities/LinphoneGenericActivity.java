...
public abstract class LinphoneGenericActivity extends ThemeableActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        prt(getClass().getSimpleName() + "(LinGenAct) onCreate");
    ...
    protected void onResume() {
        prt(getClass().getSimpleName() + "(LinGenAct).onResume");
        super.onResume();
        curAct = this;
    ...
    public static void setLED(char mode, int rgb) {
        // Command:  L 0xff0000   -- set color red solid
        //   P 0x00FF00   -- set pulsing green
        //   M 1         -- Turn on red circle led around moute button
        //   R           -- Go into rainbow mode
        String cmd;
        if (mode == 'L' || mode == 'P') { // Led color, or Pulsing led color
            cmd = mode + " " + (rgb >> 16 & 0xff) + " " + (rgb >> 8 & 0xff) + " " + (rgb & 0xff);
        } else if (mode == 'M') { // Mute button led on/off
            cmd = "M " + (rgb == 0 ? "0" : "1");
        } else {
            cmd = "R";
        }
        call("su -c lights " + cmd);
    } // end of setLED
    ...
    public static String call(String cmd, String inputStr) {
        try {
            ...
            if (inputStr != null) {
                ...
                procIn.write(inputStr.getBytes());
                procIn.close();
            }
            proc.waitFor();
            int rc = proc.exitValue();
            int bAvail = proc.getInputStream().available();
            byte[] ba = new byte[bAvail];
            proc.getInputStream().read(ba);
            String resp = new String(ba);
            prt("rc=" + rc + " " + resp);
            return resp;
        } catch (IOException ex) {
            prt("Caught exception running: " + cmd + ", ex: " + ex);
